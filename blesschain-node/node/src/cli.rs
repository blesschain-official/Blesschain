...
/// Supported consensus engines for development.
#[derive(Debug, Clone)]
pub enum Consensus {
    ManualSeal(u64),
    InstantSeal,
...
/// The primary CLI structure for BlessChain node.
#[derive(Debug, Parser)]
pub struct Cli {
    #[command(subcommand)]
...
/// All supported subcommands (based on sc-cli defaults).
...
pub enum Subcommand {
    ...
    /// (Deprecated) Build a chain specification.
    #[deprecated(
    ...
    /// Export state for a given block.
    ExportState(sc_cli::ExportStateCmd),

    /// Import blocks.
    ImportBlocks(sc_cli::ImportBlocksCmd),

    /// Remove the entire chain data directory.
    PurgeChain(sc_cli::PurgeChainCmd),

    /// Revert the chain to a previous state.
    Revert(sc_cli::RevertCmd),

    /// Display DB column metadata.
