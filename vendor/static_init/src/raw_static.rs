...
mod static_impl {
    ...
    impl<T> Static<T> {
        ...
        #[inline]
        /// # Safety
        /// this shall referer to Self and shall not
        /// be used when the variable is otherwise borrowed
        ...
        if status == 0 {
            core::panic!(
                "Attempt to access variable {:#?} before it is initialized during initialization \
                 priority {prio}. Tip: increase init priority of this static to a value larger than \
...
mod static_impl {
    ...
    impl<T> Static<T> {
        ...
        #[inline]
        /// # Safety
        /// The object shall not already be borrowed
        pub fn set_to(this: &mut Self, v: T) {
                this.0.write(v);
        }
        ...
        pub unsafe fn drop(this: &mut Self) {
            unsafe {
                // SAFETY:
                // as long as it has been initialized, => requirement from safety section
                // and the static is not borowed otherwise => requirement from the use of 
                // the static
                core::ptr::drop_in_place(this.0.as_mut_ptr());
            }
        }
    }
    ...
    impl<T> ConstStatic<T> {
        ...
        /// # Safety
        ///
        /// this shall no already be borrowed
        pub fn set_to(this: &mut Self, v: T) {
            Static::<T>::set_to(&mut this.0, v) 
        }
        #[inline]
        /// # Safety
        ///
