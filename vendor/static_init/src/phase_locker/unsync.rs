...
impl<T> Deref for UnSyncPhaseGuard<'_, T> {
    type Target = T;
    #[inline(always)]
    fn deref(&self) -> &T {
        self.0
...
impl<T> Phased for UnSyncPhaseGuard<'_, T> {
...
impl<T: ?Sized> Drop for UnSyncPhaseGuard<'_, T> {
    #[inline(always)]
    fn drop(&mut self) {
...
impl<T> Deref for UnSyncReadPhaseGuard<'_, T> {
    type Target = T;
    #[inline(always)]
    fn deref(&self) -> &T {
        self.0
...
impl<T> Phased for UnSyncReadPhaseGuard<'_, T> {
    fn phase(this: &Self) -> Phase {
        this.phase()
    }
}
impl<T> Clone for UnSyncReadPhaseGuard<'_, T> {
    fn clone(&self) -> Self {
...
impl<T: ?Sized> Drop for UnSyncReadPhaseGuard<'_, T> {
    #[inline(always)]
    fn drop(&mut self) {
...
impl UnSyncPhaseLocker {
    ...
    pub fn lock_mut<'a, T: ?Sized>(&'a mut self, v: &'a T) -> UnSyncPhaseGuard<'a, T> {
        self.0.set(self.0.get() | LOCKED_BIT);
